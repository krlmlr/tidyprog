...
```{r setup-34-function-purity-r}
```


<!--Functions should do one thing, and do it well. -->
```{r functions-should-do-one-thing-and-do-it-well}
```


<!--Pure function: called for its return value, no side effects! -->
```{r pure-function-called-for-its-return-value-no-side-effects}
...
```{r function-with-side-effect-called-for-its-side-effect-returns-input}
```


<!--invisibly (=no changes to return value). -->
```{r invisibly-no-changes-to-return-value}
...
```{r separation-helps-isolate-the-side-effects}
```


<!--If side effect functions return the, they remain composable with -->
```{r if-side-effect-functions-return-the-they-remain-composable-with}
```
