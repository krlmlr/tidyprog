...
```
```{r 36-setup-36-function-details-r}
library(tidyverse)
library(here)
library(conflicted)
...
```
```{r 36-function-arguments-can-have-default-values}
...
```
```{r 36-function-arguments-can-be-called-by-name-or-by-position}
...
```
```{r 36-multiple-function-arguments-are-separated-by-comma}
...
```
```{r 36-ellipsis-variable-argument-list-useful-for-passing-arguments-downstream}
...
```
```{r 36-note-despite-many-changes-and-new-features}
read_weather_data()
```
...
```
```{r 36-what-does-the-following-return-why}
...
```
```{r 36-how-do-you-avoid-this-behavior}
