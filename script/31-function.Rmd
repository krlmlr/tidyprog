...
```{r important-please-reset-the-r-session-before-running-this-script}
```


<!-- (In RStudio: Session/Restart R) -->
```{r in-r-studio-session-restart-r}
```


<!-- Declare a function to hide implementation details -->
```{r declare-a-function-to-hide-implementation-details}
...
```{r execute-like-functions-exported-from-packages}
read_weather_data()
```


<!-- Variables are local, execution doesn't change global variables. -->
```{r variables-are-local-execution-doesn-t-change-global-variables}
```


<!-- (The effect is only seen when starting R in a fresh session.) -->
```{r the-effect-is-only-seen-when-starting-r-in-a-fresh-session}
ls()
```


<!-- Assign result to use in further operations -->
```{r assign-result-to-use-in-further-operations}
weather_data <- read_weather_data()
```


<!-- Functions can have arguments -->
```{r functions-can-have-arguments}
...
```{r functions-can-be-called-by-other-functions}
read_weather_data <- function() {
  # Read all files
...
```{r still-need-to-run-a-function}
read_weather_data()
```


<!-- Functions can help eliminate intermediate variables -->
```{r functions-can-help-eliminate-intermediate-variables}
